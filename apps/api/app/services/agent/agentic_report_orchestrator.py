...
class AgenticReportOrchestrator:
    ...
    Executes the 16-step One-Click Auto Report Pipeline:
    ...
    async def run_workflow(
        ...
        async def check_job_state() -> Optional[str]:
            """Returns 'cancelled' or 'paused' or None."""
            fresh_job = await job_repo.get(db, job_id)
            if not fresh_job:
                return "cancelled"
            if fresh_job.status in ["cancelled", "failed"]:
                return "cancelled"
        ...
        try:
            ...
            if data_files:
                ...
                try:
                    dataset_summary = data_engine.profile_dataset(data_files[0].file_path)
                except Exception:
                    pass
            else:
